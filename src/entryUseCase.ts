...
export const createAll = (getClient: () => Promise<PoolClient>) => (
  ...
  return A.array.sequence(TE.taskEitherSeq)(
    entries.map((entry) => createOne(getClient)(entry))
  );
};

export const readAll = (getClient: () => Promise<PoolClient>) => (
  limit: number
): TE.TaskEither<any, Entry[]> => async () => {
  const dbInvoker = entriesRepository.selectAll(await getClient());
  const result = await dbInvoker(limit);
  return E.right(result);
};

export const readOne = (getClient: () => Promise<PoolClient>) => (
  uuid: string
): TE.TaskEither<any, Entry> => async () => {
...
export const deleteAll = (getClient: () => Promise<PoolClient>) => (
  ...
  return A.array.sequence(TE.taskEitherSeq)(
    uuids.map((entry) => deleteOne(getClient)(entry))
  );
};
